...
router.post('/persons', function (req, res) {
    let Array = []
    // let persons = req.body.personsdata
    let voting_age = req.query.age
    console.log(voting_age)
    for(let i = 0 ; i< personsdata.length ; i++){
        if(personsdata[i].age>voting_age){
            personsdata[i].votingStatus = true;
            Array.push(personsdata[i])
        }
    }
    console.log(Array)
    res.send({Array})
